...
rnd_bg_event_location <- function (sigma_bg=1) {
  Mode <- sample(1:3,size=1)
  Means <- 5/sqrt(2)*matrix(c(0,1,1,0,(1+sqrt(3))/2,(1+sqrt(3))/2),3,2,byrow=TRUE) # equidistant points
  location <- rnorm(n=2,mean = Means[Mode,],sd=sigma_bg)
  return(location)
} 

rnd_bg_event_time <- function (tau_bg=1) {
  Mode <- sample(1:2,size=1)
  Means <- c(1,7) # equidistant points
...
rnd_offspring <- function(m=20,Gl) { # Gi coordinate vector (x,y,t)
  ...
  if (count==0) {
    ...
    return(c(locations,times)) # TODO only return when no NAs
  } else {
...
next_gen <- function(prev_gen,m=20) {
  if( !is.null(nrow(prev_gen)) ) {
    children <- matrix(rnd_offspring(m=m,prev_gen[1,]),1,3)
    if(nrow(prev_gen)>1) {
      for(i in 2:nrow(prev_gen)) {
        newChildren <- rnd_offspring(m=m,prev_gen[i,])
...
generate_sample <- function(mean_N_bg_points=40,m=0.5) {
  ...
  generations <- list()
  generations[[1]] <- generate_bg_events(N=N_bg_points)
  i <- 1
  while( sum(complete.cases(generations[[i]]))>0 ) {
    i <- i + 1
    cat(dim(generations[[i-1]]))
